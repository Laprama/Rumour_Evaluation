...
def get_embeddings(embs_model, train_records):
    ...
    for i, record in enumerate(train_records):
        cq_embs[i] = get_embedding(embs_model, record["context"] + " " + record["question"])
...
def main(train_file, input_file, output_file):
    ...
    embs_model = SentenceTransformer('roberta-large-nli-stsb-mean-tokens')  # 'distilbert-base-nli-mean-tokens')
    ...
    if os.path.exists("traincqembs.txt"):
        ...
        cq_embs, A_embs, B_embs, C_embs, train_labels = get_embeddings(embs_model, train_records)
    ...
    # train a classifier
    classifier = LogisticRegression().fit(joint_embs, joint_labels)
    ...
    if os.path.exists("testcqembs.txt"):
        ...
        cq_embs, A_embs, B_embs, C_embs, test_labels = get_embeddings(embs_model, test_records)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-file', type=str, required=True, help='Location of training records', default=None)
